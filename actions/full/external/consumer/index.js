...
/**
 * Handles the incoming full sync event and routes it to the appropiate runtime action.
 *
 * @param {FullSyncActions.ConsumerEnv} params - The environment parameters.
 * @returns {Promise<ActionResponse>} The response of the action.
 */
const main = async params => {
  const logger = Core.Logger('full-sync-external-consumer', {
  ...
  try {
    ...
    switch (params.type) {
      case 'sfcc.full.sync': {
        logger.info('Invoking full sync');
        activationId = await openwhiskClient.invokeAction('full-backoffice/sync', params.data);
        break;
      }
      default: {
