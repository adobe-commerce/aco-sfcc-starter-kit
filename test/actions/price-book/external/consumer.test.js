...
const action = require('../../../../actions/price-book/external/consumer/index.js');
...
describe('price-book-external-consumer', () => {
  ...
  test('should set logger to use LOG_LEVEL param', async () => {
    ...
    expect(Core.Logger).toHaveBeenCalledWith('price-book-external-consumer', { level: 'debug' });
  });

  test('should handle sfcc.price-book.sync event type and invoke sync action', async () => {
    const mockOpenwhiskInstance = {
      invokeAction: jest.fn().mockResolvedValue('activation-456'),
    };
    Openwhisk.mockReturnValue(mockOpenwhiskInstance);

    const testData = { siteId: 'test-site-123' };
    ...
    expect(mockOpenwhiskInstance.invokeAction).toHaveBeenCalledWith('priceBook-backoffice/sync', testData);
    expect(response).toEqual({
      statusCode: 200,
      body: {
        type: 'sfcc.price-book.sync',
        response: { activationId: 'activation-456' },
      },
    });
  });
  ...
  test('should return 500 when action invocation fails', async () => {
    ...
    expect(response).toEqual({
      error: {
        statusCode: 500,
        body: { error: 'Error invoking action: sfcc.price-book.sync' },
      },
    });
    expect(mockLoggerInstance.error).toHaveBeenCalledWith('Error invoking action: sfcc.price-book.sync');
  });
