...
/**
 * Handles the incoming metadata sync event and routes it to the appropiate runtime action.
 *
 * @param {MetadataActions.ConsumerEnv} params - The environment parameters.
 * @returns {Promise<ActionResponse>} The response of the action.
 */
const main = async params => {
  const logger = Core.Logger('metadata-external-consumer', {
  ...
  try {
    ...
    switch (params.type) {
      case 'sfcc.metadata.sync': {
        logger.info('Invoking metadata sync');
        activationId = await openwhiskClient.invokeAction('metadata-backoffice/sync', params.data);
        break;
      }
      default: {
