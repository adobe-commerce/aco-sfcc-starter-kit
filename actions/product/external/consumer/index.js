...
/**
 * Handles the incoming product sync event and routes it to the appropiate runtime action.
 *
 * @param {ProductActions.ConsumerEnv} params - The environment parameters.
 * @returns {Promise<ActionResponse>} The response of the action.
 */
const main = async params => {
  const logger = Core.Logger('product-external-consumer', {
  ...
  try {
    ...
    switch (params.type) {
      case 'sfcc.product.sync': {
        logger.info('Invoking product sync');
        activationId = await openwhiskClient.invokeAction('product-backoffice/sync', params.data);
        break;
      }
      default: {
