...
const action = require('../../../../actions/category/external/consumer/index.js');
...
describe('category-external-consumer', () => {
  ...
  test('should set logger to use LOG_LEVEL param', async () => {
    ...
    expect(Core.Logger).toHaveBeenCalledWith('category-external-consumer', { level: 'debug' });
  });

  test('should handle sfcc.category.sync event type and invoke sync action', async () => {
    const mockOpenwhiskInstance = {
    ...
    expect(mockOpenwhiskInstance.invokeAction).toHaveBeenCalledWith('category-backoffice/sync', testData);
  ...
  test('should return 500 when action invocation fails', async () => {
    ...
    expect(response).toEqual({
      error: {
        statusCode: 500,
        body: { error: 'Error invoking action: sfcc.category.sync' },
      },
    });
    expect(mockLoggerInstance.error).toHaveBeenCalledWith('Error invoking action: sfcc.category.sync');
  });
