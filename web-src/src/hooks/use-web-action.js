...
export function formatRelativeTime(lastChecked) {
  ...
  return new Intl.RelativeTimeFormat('en-US').format(-Math.round(diffInSeconds / 60), 'minutes');
}
 ...
 */
export const useWebAction = (name, params = {}) => {
  const ims = useIms();
  ...
  async function invokeAction(overrideParams = params) {
    ...
    const devHeaders = window.location.hostname === 'localhost' ? { 'x-ow-extra-logging': 'on' } : {};

    try {
      const { method = 'POST', headers, ...rest } = deepmerge(params, overrideParams);
