...
const action = require('../../../../actions/product/external/consumer/index.js');
...
describe('product-external-consumer', () => {
  ...
  test('should set logger to use LOG_LEVEL param', async () => {
    ...
    expect(Core.Logger).toHaveBeenCalledWith('product-external-consumer', { level: 'debug' });
  });

  test('should handle sfcc.product.sync event type and invoke sync action', async () => {
    const mockOpenwhiskInstance = {
      invokeAction: jest.fn().mockResolvedValue('activation-123'),
    };
    Openwhisk.mockReturnValue(mockOpenwhiskInstance);

    const testData = { productId: 'test-product-123' };
    ...
    expect(mockOpenwhiskInstance.invokeAction).toHaveBeenCalledWith('product-backoffice/sync', testData);
    expect(response).toEqual({
      statusCode: 200,
      body: {
        type: 'sfcc.product.sync',
        response: { activationId: 'activation-123' },
      },
    });
  });
  ...
  test('should return 500 when action invocation fails', async () => {
    ...
    expect(response).toEqual({
      error: {
        statusCode: 500,
        body: { error: 'Error invoking action: sfcc.product.sync' },
      },
    });
    expect(mockLoggerInstance.error).toHaveBeenCalledWith('Error invoking action: sfcc.product.sync');
  });
