...
/**
 * Handles the incoming delta sync event and routes it to the appropiate runtime action.
 *
 * @param {DeltaSyncActions.ConsumerEnv} params - The environment parameters.
 * @returns {Promise<ActionResponse>} The response of the action.
 */
const main = async params => {
  const logger = Core.Logger('delta-sync-external-consumer', {
  ...
  try {
    ...
    switch (params.type) {
      case 'sfcc.delta.sync': {
        logger.info('Invoking delta sync');
        activationId = await openwhiskClient.invokeAction('delta-backoffice/sync', params.data);
        break;
      }
      default: {
