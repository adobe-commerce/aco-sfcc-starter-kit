...
/**
 * Handles the incoming price book sync event and routes it to the appropiate runtime action.
 *
 * @param {PriceBookActions.ConsumerEnv} params - The environment parameters.
 * @returns {Promise<ActionResponse>} The response of the action.
 */
const main = async params => {
  const logger = Core.Logger('price-book-external-consumer', {
  ...
  try {
    ...
    switch (params.type) {
      case 'sfcc.price-book.sync': {
        logger.info('Invoking price book sync');
        activationId = await openwhiskClient.invokeAction('priceBook-backoffice/sync', params.data);
        break;
      }
      default: {
