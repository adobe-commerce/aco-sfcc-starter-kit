...
const action = require('../../../../actions/metadata/external/consumer/index.js');
...
describe('metadata-external-consumer', () => {
  ...
  test('should set logger to use LOG_LEVEL param', async () => {
    ...
    await action.main({
      ...
      data: { locales: 'en_US,fr_FR' },
    });

    expect(Core.Logger).toHaveBeenCalledWith('metadata-external-consumer', { level: 'debug' });
  });

  test('should handle sfcc.metadata.sync event type and invoke sync action', async () => {
    const mockOpenwhiskInstance = {
      invokeAction: jest.fn().mockResolvedValue('activation-789'),
    };
    Openwhisk.mockReturnValue(mockOpenwhiskInstance);

    const testData = { locales: 'en_US,fr_FR' };
    ...
    expect(mockOpenwhiskInstance.invokeAction).toHaveBeenCalledWith('metadata-backoffice/sync', testData);
    expect(response).toEqual({
      statusCode: 200,
      body: {
        type: 'sfcc.metadata.sync',
        response: { activationId: 'activation-789' },
      },
    });
  });
  ...
  test('should return 500 when action invocation fails', async () => {
    ...
    expect(response).toEqual({
      error: {
        statusCode: 500,
        body: { error: 'Error invoking action: sfcc.metadata.sync' },
      },
    });
    expect(mockLoggerInstance.error).toHaveBeenCalledWith('Error invoking action: sfcc.metadata.sync');
  });
